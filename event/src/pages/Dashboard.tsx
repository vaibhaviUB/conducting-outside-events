...
import {
  LayoutDashboard, CalendarDays, Compass, Bell, User, LogOut, Brain, Search,
  TrendingUp, Sparkles, Zap, Target, ChevronRight, Smile, Meh, Frown,
  MapPin, Clock, Heart, MessageCircle, Award, Mail, Settings, Check,
} from "lucide-react";
    ...
    <div className="relative min-h-screen flex">
      ...
      <div className="flex-1 min-w-0 pb-20 lg:pb-0">
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
          ...
          {active === "profile" && (
            <ProfileView
              onEdit={() => toast({ title: "Edit profile", description: "Coming soon — profile editor." })}
            />
          )}
        </div>
      </div>
...
const MyEventsView = forwardRef<HTMLElement, any>(({ events, onToggleStatus, onRemove }, ref) => (
  <section ref={ref} className="glass rounded-3xl p-6 sm:p-8 animate-fade-in-up">
    ...
    {events.length === 0 ? (
      <p className="mt-6 text-sm text-muted-foreground">No events yet. Head to Explore to add some.</p>
    ) : (
...
const ProfileView = forwardRef<HTMLDivElement, any>(({ onEdit }, ref) => (
  <div ref={ref} className="space-y-6">
    <section className="glass rounded-3xl p-6 sm:p-8 relative overflow-hidden animate-fade-in-up">
      <div className="absolute -top-20 -right-20 h-64 w-64 rounded-full bg-neon-purple/20 blur-3xl" />
      <div className="relative flex flex-col sm:flex-row sm:items-center gap-5">
        ...
        <div className="flex-1">
          <h1 className="font-display text-2xl font-semibold">Aarav Mehta</h1>
          <p className="text-sm text-muted-foreground">Computer Science · Year 2</p>
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="text-[10px] font-mono uppercase rounded-full px-2 py-0.5 bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/30">wellness</span>
            <span className="text-[10px] font-mono uppercase rounded-full px-2 py-0.5 bg-neon-purple/10 text-neon-purple border border-neon-purple/30">cultural</span>
            <span className="text-[10px] font-mono uppercase rounded-full px-2 py-0.5 bg-neon-pink/10 text-neon-pink border border-neon-pink/30">outdoor</span>
          </div>
        </div>
    ...
    <div className="grid gap-6 sm:grid-cols-3">
      {[
        ...
        { label: "Wellness score", value: "8.4" },
      ].map((s, i) => (
        <div key={s.label} className="glass rounded-2xl p-5 text-center animate-fade-in-up" style={{ animationDelay: `${0.05 + i * 0.05}s` }}>
